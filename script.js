...
const displayMessage = function (tahmin) {
  document.querySelector(".tahmin").textContent = tahmin;
};
document.querySelector(".check").addEventListener("click", function () {
  const guess = Number(document.querySelector(".guess").value);
  if (guess < 1 || guess > 100) {
    alert("1-100 arasında bir sayı giriniz");
  } else if (!guess) {
    displayMessage("lütfen Bir Tahminde Bulunun");
  } else if (guess == secretNumber) {
      displayMessage("Tebrikler");
  } else if (guess !== secretNumber) {
    if (score > 1) {
      displayMessage(
        guess > secretNumber ? "Tahmini düşürün" : "Tahmini yükseltin"
      );
      attempt++;
      score--;
